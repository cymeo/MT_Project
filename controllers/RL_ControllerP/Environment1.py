...
class WeBot_environment(Env):
    ...
    def __init__(self):
        ######### rewards for -distance to goal,-rotational_distance, success, crash ############
        self.weights = np.array([0.07,0.03,5,10]) 
        self.max_step = 60
        super().__init__()
    ...
    def get_reward(self): 
        ...
        if (self.dist <= 0.03):
            ...
            self.done = True 
        if (self.dist<=0.03 and self.rot_dist <= 0.1): 
            R_success += 0.2 
            print("rot_success")
