...
def plot_monitor_data(file_path):
    ...
    #stackplot
    window_size = 100  
    crash_count = np.convolve(rewards <= -100, np.ones(window_size, dtype=int), mode='same')
    success_count = np.convolve(rewards > 0, np.ones(window_size, dtype=int), mode='same')
    overstepped_count = np.convolve((-100 < rewards) & (rewards < 0), np.ones(window_size, dtype=int), mode='same')    
...
# Example usage
plot_monitor_data('/home/cecily/MasterThesis_Cy/MT_Project/controllers/RL_Controller/monitor_logs/env_03_41.monitor.csv')
