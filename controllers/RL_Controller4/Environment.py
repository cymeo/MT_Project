...
class WeBot_environment(Env):
    ...
    def __init__(self):
        ...
        self.weights = np.array([0.2,0.0,5,10]) 
        self.max_step = 400
        super().__init__()
        # Define action and observation space
        #action = Motorangle steps  
        self.action_space = spaces.Box(low= -2/180*np.pi, high = 2/180*np.pi, shape = (6,))
    ...
    def get_observation(self):
        ...
        p_ee = FW.get_forward_kinematics(self.theta)
        transl = np.array(p_ee[:3,3])
        rot= R.from_matrix(p_ee[:3,:3])
   ...
   #returne done and if successed 
    def check_done(self):     
        ...
        if (self.dist <= 0.05):
            success = True
            print('success, distance: ', self.current_step )
            return True, True
        # step mlimit reached           
    ...
    def step(self, action):
        ...
        sensors, self.crashed = FW.move_robot(new_theta) 
